...
const tabsStyles =
  'h-8 sm:h-6 md:h-8 lg:h-10 xl:16 2xl:h-16 dark:text-slate-200 text-slate-100';
        ...
        <div className="grid grid-cols-12 rounded-none">
          <div className="col-span-1 bg-gradient-to-t from-amber-200 to-amber-400 dark:from-slate-700 dark:to-slate-900">
            {tabButtons.map(({ title, icon, path }, index) => (
              ...
              >
                <div>{icon}</div>
                <span className="hidden text-sm font-medium text-slate-100 dark:text-slate-200 lg:block">
                  {title}
                </span>
              </NavLink>
            ))}
